...
class SimpleCipher:
    ...
    def xor_crypt(self, data: str) -> str:
        encoded = []
        for i, c in enumerate(data.encode()):
            key_c = self.key[i % len(self.key)]
            encoded.append(c ^ key_c)
        return bytes(encoded).hex()

# Clave y contraseña compatibles
CIPHER = SimpleCipher(key="TEC_2025")  # Mantén esta clave

USERS = {
    "admin": {
        "password": "24372c265753465a65",  # Nuevo hash válido
        "role": "admin"
    }
}
...
def login():
    if request.method == 'POST':
        ...
        encrypted_password = CIPHER.xor_crypt(password)
...
def control_all_lights():
    ...
    try:
        ...
        for pin in LightController.LIGHT_PINS.values():
...
@app.route('/api/doors/<int:door_id>', methods=['POST'])
@login_required
def control_door(door_id):
    # Solo funciona en modo simulado
    if not isinstance(GPIO, MockGPIO):
        return jsonify({"error": "Función solo disponible en simulación"}), 403
    ...
    # Verificar si la puerta existe
    pin = DoorMonitor.DOOR_PINS.get(door_id)
    if not pin:
        return jsonify({"error": "Puerta no encontrada"}), 404

    # Simular cambio de estado (pull-up invertido)
    GPIO.output(pin, GPIO.LOW if state == 'open' else GPIO.HIGH)
